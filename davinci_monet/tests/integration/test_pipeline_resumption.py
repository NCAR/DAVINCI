...
def _analysis_source(path: Path) -> None:
    ...
    oscillation = np.sin(2.0 * np.pi * time / 16.0)
    pattern = np.cos(np.deg2rad(lat))[:, None] * np.cos(np.deg2rad(lon))[None, :]
    field = oscillation[:, None, None] * pattern[None, :, :]
...
def test_analysis_dag_resumes_after_first_node(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert reference.context is not None
    expected = {
        name: scientific_dataset_sha256(reference.context.sources[name].data)
        for name in ("basis", "wavelet")
    }

    original_capture = CheckpointManager.capture_source
    interrupted = False

    def interrupt_after_basis(self, request, value, **kwargs):  # noqa: ANN001
        nonlocal interrupted
        receipt = original_capture(self, request, value, **kwargs)
        if request.stage == "analyses" and request.item == "basis" and not interrupted:
            interrupted = True
            raise KeyboardInterrupt
        return receipt

    monkeypatch.setattr(CheckpointManager, "capture_source", interrupt_after_basis)
    ...
    assert resumed.context is not None
    assert list(resumed.context.sources) == ["model", "basis", "wavelet"]
    actual = {
    ...
    manager = resumed.context.checkpoint_manager
    assert manager is not None
    basis_receipt = manager.store.read_receipt("analyses", "basis")
    assert basis_receipt is not None
    assert basis_receipt.generation == 1


def test_analysis_adopts_finalized_artifact_before_recomputing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = tmp_path / "adopt_analysis.nc"
    _analysis_source(source)
    config = _analysis_config(tmp_path / "a012", source)
    stages = [LoadSourcesStage(), AnalysesStage()]
    original_analyze = EOFAnalysis.analyze_inputs
    original_capture = CheckpointManager.capture_source
    calls = 0
    interrupted = False

    def counted_analyze(self, inputs, spec, runtime):  # noqa: ANN001
        nonlocal calls
        calls += 1
        return original_analyze(self, inputs, spec, runtime)

    def interrupt_before_receipt(self, request, value, **kwargs):  # noqa: ANN001
        nonlocal interrupted
        if request.stage == "analyses" and request.item == "basis" and not interrupted:
            interrupted = True
            raise KeyboardInterrupt
        return original_capture(self, request, value, **kwargs)

    monkeypatch.setattr(EOFAnalysis, "analyze_inputs", counted_analyze)
    monkeypatch.setattr(CheckpointManager, "capture_source", interrupt_before_receipt)
    with pytest.raises(KeyboardInterrupt):
        PipelineRunner(stages=stages, show_progress=False).run_from_config(config)

    artifact = tmp_path / "a012" / "output" / "artifacts" / "basis" / "summary.json"
    ...
    assert manager is not None
    store = manager.store
    assert store.read_receipt("analyses", "basis") is None
    ...
    assert calls == 1
    assert resumed.context is not None
    entries = resumed.context.metadata["analysis_artifacts"]
    assert any(
        entry.get("analysis") == "basis" and entry.get("publication") == "adopted"
        for entry in entries
    )
    manager = resumed.context.checkpoint_manager
    assert manager is not None
    receipt = manager.store.read_receipt("analyses", "basis")
    assert receipt is not None
    assert all("/output/artifacts/basis/" in path for path in receipt.objects[0].paths)
