...
def test_dataset_codec_discards_decoded_hdf_packing_without_changing_values(
    tmp_path: Path,
) -> None:
    codecs = CheckpointCodecs(tmp_path / "a001")
    source = xr.Dataset(
    ...
    try:
        ...
        for name in restored.data_vars:
            assert not set(packing) & set(restored[name].attrs)
    finally:
        restored.close()

    for name in source.data_vars:
        assert set(packing) <= set(source[name].attrs)
        assert set(packing) <= set(source[name].encoding)
