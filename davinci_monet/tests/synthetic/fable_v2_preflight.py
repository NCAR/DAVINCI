"""Frozen one-seed preflight for the selected FABLE v2 policy."""

from __future__ import annotations

import hashlib
import hmac
import json
import os
import tempfile
import time
from collections.abc import Callable, Mapping
from dataclasses import dataclass, replace
from pathlib import Path
from typing import Any

from davinci_monet.tests.synthetic._aerosol_contracts import SyntheticTuningSpec
from davinci_monet.tests.synthetic.fable_acceptance_gate import resource_gate
from davinci_monet.tests.synthetic.fable_v2_attempts import (
    claim_v2_phase_attempt,
    validate_v2_phase_attempt,
)
from davinci_monet.tests.synthetic.fable_v2_bundle import generate_v2_scenario_bundle
from davinci_monet.tests.synthetic.fable_v2_calibration import (
    V2RecoverySeedResult,
    recovery_result_failures,
)
from davinci_monet.tests.synthetic.fable_v2_calibration_record import (
    load_v2_calibration_record,
    validate_v2_calibration_record,
)
from davinci_monet.tests.synthetic.fable_v2_evidence import validate_v2_scenario_evidence
from davinci_monet.tests.synthetic.fable_v2_freeze import (
    FrozenFileIdentity,
    FrozenPreregistration,
    V2Preregistration,
)
from davinci_monet.tests.synthetic.fable_v2_lifecycle import (
    V2GenerationPrerequisites,
    prepare_v2_generation,
)
from davinci_monet.tests.synthetic.fable_v2_lock_evidence import (
    validate_v2_generation_lock_identity,
)
from davinci_monet.tests.synthetic.fable_v2_policy import (
    FableV2Policy,
    v2_policy_from_normalized,
)
from davinci_monet.tests.synthetic.fable_v2_protocol import (
    CYCLE_ID,
    protocol_sha256,
    seed_roles,
    validate_role_seeds,
)
from davinci_monet.tests.synthetic.fable_v2_rejection import (
    V2FailureClassification,
    rejection_failure,
    write_v2_phase_rejection,
)
from davinci_monet.tests.synthetic.fable_v2_runner import (
    V2ScenarioOutcome,
    run_v2_scenario,
)

V2_PREFLIGHT_SCHEMA = "fable-v2-preflight-v1"
ScenarioRunner = Callable[..., V2ScenarioOutcome]


@dataclass(frozen=True)
class V2PreflightRecord:
    """Immutable disposition for the selected policy's single preflight run."""

    preregistration: FrozenPreregistration
    attempt: FrozenFileIdentity
    generation_lock: FrozenFileIdentity
    calibration_record: FrozenFileIdentity
    selected_policy: FableV2Policy
    result: V2RecoverySeedResult

    def __post_init__(self) -> None:
        validate_role_seeds("preflight", (self.result.seed,))
        validate_v2_scenario_evidence(
            _evidence_mapping(self.result.evidence_json),
            SyntheticTuningSpec.synthetic_osse(self.result.seed),
            self.selected_policy,
            score_kind="recovery",
            evaluation_splits=("development_test",),
            verify_files=False,
        )

    @property
    def failures(self) -> tuple[str, ...]:
        return recovery_result_failures(self.result)

    @property
    def status(self) -> str:
        return "passed" if not self.failures else "failed"

    def normalized(self) -> dict[str, Any]:
        return {
            "calibration_record": self.calibration_record.normalized(),
            "attempt": self.attempt.normalized(),
            "generation_lock": self.generation_lock.normalized(),
            "cycle_id": CYCLE_ID,
            "failures": list(self.failures),
            "preregistration": self.preregistration.normalized(),
            "protocol_sha256": protocol_sha256(),
            "result": self.result.normalized(),
            "schema_version": V2_PREFLIGHT_SCHEMA,
            "selected_policy": self.selected_policy.normalized(),
            "status": self.status,
        }


def write_v2_preflight_record(path: str | Path, record: V2PreflightRecord) -> Path:
    """Atomically publish one read-only passing or failed preflight result."""
    destination = Path(path).expanduser().resolve()
    value = record.normalized()
    _write_once(
        destination,
        _canonical_json({"record": value, "record_sha256": _json_sha256(value)}) + "\n",
    )
    return destination


def load_v2_preflight_record(path: str | Path) -> V2PreflightRecord:
    """Load canonical bytes and recompute the preflight disposition."""
    source = Path(path).expanduser().resolve()
    raw = source.read_text(encoding="ascii")
    try:
        envelope = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise ValueError("invalid v2 preflight JSON") from exc
    if (
        not isinstance(envelope, Mapping)
        or set(envelope) != {"record", "record_sha256"}
        or raw != _canonical_json(envelope) + "\n"
    ):
        raise ValueError("v2 preflight record is not canonically encoded")
    value = _mapping(envelope["record"], "preflight record")
    if not hmac.compare_digest(str(envelope["record_sha256"]), _json_sha256(value)):
        raise ValueError("v2 preflight record SHA-256 mismatch")
    record = _record_from_normalized(value)
    if record.normalized() != value:
        raise ValueError("v2 preflight record scientific disposition mismatch")
    return record


def validate_v2_preflight_record(
    record: V2PreflightRecord,
    current: V2Preregistration,
    frozen: FrozenPreregistration,
    calibration_identity: FrozenFileIdentity,
) -> V2PreflightRecord:
    """Reverify every prerequisite and selected policy bound into preflight."""
    calibration_identity.verify()
    calibration = validate_v2_calibration_record(
        load_v2_calibration_record(calibration_identity.path), current, frozen
    )
    if record.preregistration.normalized() != frozen.normalized():
        raise ValueError("preflight uses a different preregistration")
    if record.calibration_record.normalized() != calibration_identity.normalized():
        raise ValueError("preflight uses a different calibration record")
    if record.selected_policy != calibration.selected_policy:
        raise ValueError("preflight selected policy differs from frozen calibration")
    attempt_document = validate_v2_phase_attempt(record.attempt, frozen, "preflight")
    prerequisites = V2GenerationPrerequisites(
        current, frozen, calibration_record=calibration_identity
    )
    root = validate_v2_generation_lock_identity(
        record.generation_lock,
        "preflight",
        seed_roles()["preflight"],
        prerequisites,
    )
    roots = attempt_document.get("roots")
    if not isinstance(roots, Mapping) or roots.get("root") != str(root):
        raise ValueError("preflight generation lock root differs from attempt ledger")
    validate_v2_scenario_evidence(
        _evidence_mapping(record.result.evidence_json),
        SyntheticTuningSpec.synthetic_osse(record.result.seed),
        record.selected_policy,
        score_kind="recovery",
        evaluation_splits=("development_test",),
        verify_files=True,
    )
    return record


def run_v2_preflight(
    root: str | Path,
    destination: str | Path,
    current: V2Preregistration,
    frozen: FrozenPreregistration,
    calibration_path: str | Path,
    *,
    dry_run: bool = False,
    scenario_runner: ScenarioRunner = run_v2_scenario,
) -> Path:
    """Lock and execute the selected policy once on the fixed preflight seed."""
    destination_path = Path(destination).expanduser().resolve()
    if destination_path.exists():
        raise FileExistsError(f"frozen v2 preflight already exists: {destination_path}")
    calibration_identity = FrozenFileIdentity.capture(calibration_path)
    calibration = validate_v2_calibration_record(
        load_v2_calibration_record(calibration_identity.path), current, frozen
    )
    seed = seed_roles()["preflight"]
    prerequisites = V2GenerationPrerequisites(
        current,
        frozen,
        calibration_record=calibration_identity,
    )
    lock = prepare_v2_generation(root, "preflight", seed, prerequisites)
    if dry_run:
        path = lock.root / "preflight-plan.json"
        path.write_text(
            _canonical_json(
                {
                    "calibration_record": calibration_identity.normalized(),
                    "generation_lock": lock.file.normalized(),
                    "mode": "dry_run",
                    "seed": seed[0],
                    "selected_policy": calibration.selected_policy.normalized(),
                    "status": "planned",
                }
            )
            + "\n",
            encoding="ascii",
        )
        return path
    attempt = claim_v2_phase_attempt(frozen, "preflight", {"root": lock.root})
    progress_path = lock.root / "preflight-progress.json"
    progress: dict[str, Any] = {
        "attempt": attempt.normalized(),
        "seed": seed[0],
        "selected_policy_id": calibration.selected_policy_id,
        "status": "running",
    }
    _write_progress(progress_path, progress)
    try:
        spec = SyntheticTuningSpec.synthetic_osse(seed[0])
        generation_started = time.perf_counter()
        manifest = generate_v2_scenario_bundle(lock, spec)
        if Path(manifest.path) != lock.root / "bundles" / f"seed-{seed[0]}" / "scenario.json":
            raise ValueError("v2 bundle generator returned a noncanonical manifest path")
        generation_elapsed = time.perf_counter() - generation_started
        manifest_value = manifest.normalized()
    except Exception as exc:
        progress.update(error=f"{type(exc).__name__}: {exc}")
        _reject_preflight(
            destination_path,
            progress_path,
            progress,
            frozen,
            attempt,
            lock.file,
            "generation_exception",
        )
        raise
    progress["scenario_manifest"] = manifest_value
    _write_progress(progress_path, progress)
    try:
        outcome = scenario_runner(
            lock.root / "runs" / calibration.selected_policy_id / f"seed-{seed[0]}",
            spec,
            calibration.selected_policy,
            score_kind="recovery",
            lock=lock,
            evaluation_splits=("development_test",),
        )
        total_elapsed = generation_elapsed + outcome.elapsed_seconds
        outcome = replace(
            outcome,
            elapsed_seconds=total_elapsed,
            resources=resource_gate(total_elapsed, outcome.process_peak_rss_bytes),
        )
        outcome_value = outcome.normalized()
    except Exception as exc:
        progress.update(error=f"{type(exc).__name__}: {exc}")
        _reject_preflight(
            destination_path,
            progress_path,
            progress,
            frozen,
            attempt,
            lock.file,
            "pipeline_exception",
        )
        raise
    progress["outcome"] = outcome_value
    _write_progress(progress_path, progress)
    try:
        result = V2RecoverySeedResult.from_outcome(outcome)
        record = V2PreflightRecord(
            frozen,
            attempt,
            lock.file,
            calibration_identity,
            calibration.selected_policy,
            result,
        )
        validate_v2_preflight_record(record, current, frozen, calibration_identity)
    except Exception as exc:
        progress["record_error"] = f"{type(exc).__name__}: {exc}"
        _reject_preflight(
            destination_path,
            progress_path,
            progress,
            frozen,
            attempt,
            lock.file,
            "record_validation_exception",
        )
        raise
    record_path = write_v2_preflight_record(destination_path, record)
    progress.update(record=str(record_path), status=record.status)
    _write_progress(progress_path, progress)
    return record_path


def _reject_preflight(
    destination: Path,
    progress_path: Path,
    progress: dict[str, Any],
    frozen: FrozenPreregistration,
    attempt: FrozenFileIdentity,
    generation_lock: FrozenFileIdentity,
    classification: V2FailureClassification,
) -> None:
    progress.update(
        failure=rejection_failure(classification),
        record=str(destination),
        status="rejected",
    )
    write_v2_phase_rejection(
        destination,
        "preflight",
        frozen,
        attempt,
        (generation_lock,),
        classification,
        progress,
    )
    _write_progress(progress_path, progress)


def _record_from_normalized(value: Mapping[str, Any]) -> V2PreflightRecord:
    if (
        value.get("cycle_id") != CYCLE_ID
        or value.get("protocol_sha256") != protocol_sha256()
        or value.get("schema_version") != V2_PREFLIGHT_SCHEMA
    ):
        raise ValueError("v2 preflight protocol identity mismatch")
    prereg = _mapping(value["preregistration"], "preflight preregistration")
    prereg_file = _file_identity(_mapping(prereg["file"], "preregistration file"))
    frozen = FrozenPreregistration(prereg_file, str(prereg["preregistration_sha256"]))
    calibration = _file_identity(_mapping(value["calibration_record"], "calibration file"))
    attempt = _file_identity(_mapping(value["attempt"], "preflight attempt"))
    generation_lock = _file_identity(
        _mapping(value["generation_lock"], "preflight generation lock")
    )
    policy = v2_policy_from_normalized(_mapping(value["selected_policy"], "selected policy"))
    result_value = dict(_mapping(value["result"], "preflight result"))
    result = V2RecoverySeedResult(
        _canonical_json(result_value["evidence"]), str(result_value["evidence_sha256"])
    )
    if result.normalized() != result_value:
        raise ValueError("preflight result fields do not match scenario evidence")
    return V2PreflightRecord(
        frozen,
        attempt,
        generation_lock,
        calibration,
        policy,
        result,
    )


def _file_identity(value: Mapping[str, Any]) -> FrozenFileIdentity:
    return FrozenFileIdentity(str(value["path"]), str(value["sha256"]))


def _mapping(value: Any, name: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(f"{name} must be a mapping")
    return value


def _write_once(path: Path, payload: str) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists():
        raise FileExistsError(f"frozen v2 preflight already exists: {path}")
    descriptor, name = tempfile.mkstemp(prefix=f".{path.name}.", suffix=".tmp", dir=path.parent)
    temporary = Path(name)
    try:
        with os.fdopen(descriptor, "w", encoding="ascii") as stream:
            stream.write(payload)
            stream.flush()
            os.fsync(stream.fileno())
        os.chmod(temporary, 0o444)
        try:
            os.link(temporary, path)
        except FileExistsError as exc:
            raise FileExistsError(f"frozen v2 preflight already exists: {path}") from exc
    finally:
        temporary.unlink(missing_ok=True)


def _write_progress(path: Path, value: Any) -> None:
    payload = _canonical_json(value) + "\n"
    descriptor, name = tempfile.mkstemp(prefix=f".{path.name}.", suffix=".tmp", dir=path.parent)
    temporary = Path(name)
    try:
        with os.fdopen(descriptor, "w", encoding="ascii") as stream:
            stream.write(payload)
            stream.flush()
            os.fsync(stream.fileno())
        os.replace(temporary, path)
    finally:
        temporary.unlink(missing_ok=True)


def _canonical_json(value: Any) -> str:
    return json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=True)


def _json_sha256(value: Any) -> str:
    return hashlib.sha256(_canonical_json(value).encode("ascii")).hexdigest()


def _evidence_mapping(value: str) -> Mapping[str, Any]:
    parsed = json.loads(value)
    return _mapping(parsed, "preflight scenario evidence")


__all__ = [
    "V2_PREFLIGHT_SCHEMA",
    "V2PreflightRecord",
    "load_v2_preflight_record",
    "run_v2_preflight",
    "validate_v2_preflight_record",
    "write_v2_preflight_record",
]
