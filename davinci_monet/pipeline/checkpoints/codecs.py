...
def _dataset_metadata(dataset: xr.Dataset) -> dict[str, Any]:
    def variable_metadata(data: xr.DataArray) -> dict[str, Any]:
        return {
            "attrs": canonicalize(dict(data.attrs)),
...
class CheckpointCodecs:
    ...
    def reference_finalized_dataset(
        ...
        """Reference an attempt-local finalized analysis artifact without copying it."""
        summary_value = source_config.get("summary_path")
        artifact_dir = source_config.get("artifact_dir")
        artifact_path = source_config.get("artifact_path")
        if summary_value is None or (artifact_dir is None and artifact_path is None):
            return None

        summary_path = Path(str(summary_value)).expanduser().resolve()
        if artifact_dir is not None:
            root = Path(str(artifact_dir)).expanduser().resolve()
            data_paths = tuple(sorted(root.glob("chunk-*.nc")))
        else:
            data_paths = (Path(str(artifact_path)).expanduser().resolve(),)
        paths = (*data_paths, summary_path)
        try:
            for path in paths:
                path.relative_to(self.attempt_root)
        except ValueError as exc:
            raise CheckpointCodecError(
                "finalized analysis artifact is outside the attempt root"
            ) from exc
        if not data_paths or any(not path.is_file() or path.stat().st_size < 1 for path in paths):
            raise CheckpointCodecError("finalized analysis artifact is incomplete")

        checksums = {str(path): _file_sha256(path) for path in paths}
        metadata = _dataset_metadata(dataset)
        object_id = canonical_sha256(
            {
                "kind": "dataset",
                "scientific_sha256": scientific_dataset_sha256(dataset),
                "metadata": metadata,
                "artifact_checksums": {path.name: checksums[str(path)] for path in paths},
            }
        )
        return CheckpointObject(
            schema_version=OBJECT_SCHEMA_VERSION,
    ...
    def read_dataset(
        ...
        *,
        ...
        if paths:
            ...
            product_paths = [Path(path) for path in obj.paths if Path(path).name == "analysis.nc"]
            if len(product_paths) != 1:
                raise CheckpointCodecError("dataset object contains no NetCDF payload")
            dataset = load_product_dataset(product_paths[0])
